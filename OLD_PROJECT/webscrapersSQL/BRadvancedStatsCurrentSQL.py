...
def updateCurrentSeasonAdvStats():
    ...
    sqlDropTable = "DROP TABLE IF EXISTS AdvancedStats{}".format(year)

    sqlNewTable = """ CREATE TABLE IF NOT EXISTS AdvancedStats{} (
        Pk int PRIMARY KEY,
        Year int,
        Name varchar(50),
        Pos varchar(15),
    ...
    pk = 1
    for row in rowData:
        if len(row) > 0 and len(row[7]) > 0 and int(row[5]) > 5 :
            Pk = pk
            Year = year
            ...
            sqlInsert = """ 
            INSERT INTO AdvancedStats{} 
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) """.format(year)
