// The ball from public/basketball-ball-variant-svgrepo-com.svg, inlined so it
// takes the accent color and is already there the moment loading starts.
const BALL =
...
/**
 * The site's one loading indicator. A spinning basketball in the accent color,
 * sized in `em` so it scales with whatever text it sits beside.
 */
export function Spinner({
  ...
  className?: string;
  label?: string;
  /** Width and height; `em` keeps it in step with the text beside it. */
  size?: string;
}) {
  return (
    <span
      role="status"
      aria-label={label}
      className={`inline-block shrink-0 text-accent ${className}`}
